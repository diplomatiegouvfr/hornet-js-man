...
[ -z "${HB_PULLED_DIR:-}" ] && HB_PULLED_DIR="./node_modules/hornet-js-builder-new"
...
function hbw_pull_builder() {
	...
	${NPM_EXEC} install -E hornet-js-builder-new@${builderVersion} 2>&1 1>${installReportLog}
	local returnCode=$?

	if [ ! ${returnCode} -eq 0 ]; then
		log_error "Hornet-js-builder-new version ${builderVersion} is not pulled properly"
